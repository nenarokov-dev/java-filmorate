...
public class UserDataBaseTest {
    ...
    @Test
    void putUser() throws SQLException {
        userDao.setUser(user);
        userDao.putUser(userUpdate);
        User user1 = userDao.getUsersById(user.getId());
    ...
    void addFriend() throws SQLException {
        ...
        userDao.setUser(friend);
        userDao.addFriend(user.getId(), friend.getId());
        User userWithFriends = userDao.getUsersById(user.getId());
        assertTrue(userWithFriends.getFriendsId().contains(friend.getId()));
        assertTrue(friend.getFriendsId().isEmpty());
    }

    @Test
    void removeFriend() throws SQLException {
        userDao.setUser(user);
        User friend = new User(2, user.getLogin(), user.getName(), user.getEmail(), user.getBirthday());
        userDao.setUser(friend);
        userDao.addFriend(user.getId(), friend.getId());
    ...
    @Test
    void listOfFriends() throws SQLException {
        userDao.setUser(user);
        User friend = new User(2, user.getLogin(), user.getName(), user.getEmail(), user.getBirthday());
        userDao.setUser(friend);
        userDao.addFriend(user.getId(), friend.getId());
    ...
    @Test
    void commonFriends() throws SQLException {
        userDao.setUser(user);
        User user1 = new User(2, user.getLogin(), user.getName(), user.getEmail(), user.getBirthday());
        userDao.setUser(user1);
        User user2 = new User(3, user.getLogin(), user.getName(), user.getEmail(), user.getBirthday());
        userDao.setUser(user2);
        userDao.addFriend(user.getId(), user1.getId());//1 и 2 дружат
        userDao.addFriend(user1.getId(), user.getId());
        userDao.addFriend(user.getId(), user2.getId());//1 и 3 дружат
        userDao.addFriend(user2.getId(), user.getId());
    ...
    @Test
    void getFriendshipStatus() throws SQLException {
        userDao.setUser(user);
        User user1 = new User(2, user.getLogin(), user.getName(), user.getEmail(), user.getBirthday());
        userDao.setUser(user1);
        User user2 = new User(3, user.getLogin(), user.getName(), user.getEmail(), user.getBirthday());
        ...
        userDao.addFriend(user.getId(), user1.getId());//1 и 2 дружат
        userDao.addFriend(user1.getId(), user.getId());
        userDao.addFriend(user.getId(), user2.getId());//1 и дружит с 3 а 3 с 1 нет
        FriendshipStatus friendshipStatus1 = userDao.getFriendshipStatus(user.getId(), user1.getId());
        assertEquals(friendshipStatus1, FriendshipStatus.CONFIRMED);
        FriendshipStatus friendshipStatus2 = userDao.getFriendshipStatus(user.getId(), user2.getId());
